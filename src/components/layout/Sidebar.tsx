...
function DeleteTeamModal({ team, onClose, onConfirm }: { team: Team; onClose: () => void; onConfirm: () => void }) {
  ...
  const confirmPhrase = t('sidebar.deleteTeamModal.confirmPhrase')
  const expected = `${confirmPhrase} ${team.name}`
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80] p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-gray-900 dark:text-gray-100">{t('sidebar.deleteTeamModal.title')}</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t('common.irreversible')}</p>
          </div>
        </div>
        ...
        {step === 'confirm' ? (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              <span className="font-semibold text-gray-900 dark:text-gray-100">"{team.name}"</span> {t('sidebar.deleteTeamModal.description')}
            </p>
          ...
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
              {t('sidebar.deleteTeamModal.typePrompt')}
            </p>
            ...
            <input
              ...
              placeholder={`${confirmPhrase} ${team.name}`}
            ...
            <div className="flex gap-3">
              ...
              >
                {t('sidebar.deleteTeamModal.confirm')}
              </button>
            </div>
          </>
        )}
...
function DeleteProjectModal({ project, onClose, onConfirm }: { project: Project; onClose: () => void; onConfirm: () => void }) {
  ...
  const [step, setStep] = useState<'confirm' | 'type'>('confirm')
  const [typed, setTyped] = useState('')
  const confirmPhrase = t('sidebar.deleteProjectModal.confirmPhrase')
  const expected = `${confirmPhrase} ${project.name}`
  const matches = typed.trim().toLowerCase() === expected.toLowerCase()
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80] p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-gray-900 dark:text-gray-100">{t('sidebar.deleteProjectModal.title')}</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t('common.irreversible')}</p>
          </div>
        </div>
        ...
        {step === 'confirm' ? (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              <span className="font-semibold text-gray-900 dark:text-gray-100">"{project.name}"</span> {t('sidebar.deleteProjectModal.description')}
            </p>
            <div className="flex gap-3">
              <button
                onClick={onClose}
                className="flex-1 py-2 rounded-xl text-sm font-medium border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                {t('sidebar.deleteProjectModal.cancel')}
              </button>
              <button
                onClick={() => setStep('type')}
                className="flex-1 py-2 rounded-xl text-sm font-medium bg-red-600 text-white hover:bg-red-700 transition-colors"
              >
                {t('sidebar.deleteProjectModal.proceed')}
              </button>
            </div>
          </>
        ) : (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
              {t('sidebar.deleteProjectModal.typePrompt')}
            </p>
            <p className="text-sm font-mono font-semibold text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2 mb-3 select-all">
            ...
            <input
              ...
              value={typed}
              onChange={(e) => setTyped(e.target.value)}
              placeholder={`${confirmPhrase} ${project.name}`}
              className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 bg-transparent dark:text-gray-200 mb-4"
            />
            <div className="flex gap-3">
              <button
              ...
              <button
                ...
                className="flex-1 py-2 rounded-xl text-sm font-medium bg-red-600 text-white hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                {t('sidebar.deleteProjectModal.confirm')}
              </button>
            </div>
          </>
        )}
  ...
  const handleSaveTeamName = async () => {
    ...
    if (teamNameValue.trim() && teamNameValue.trim() !== team.name) {
      try {
        ...
        setTeamNameError(t('sidebar.teamNameError'))
      ...
      <div className="flex-1 overflow-y-auto py-1 scrollbar-thin">
        {isLoading ? (
          ...
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 font-medium">{t('sidebar.noTeams')}</p>
            <p className="text-xs text-gray-300 dark:text-gray-600 mt-0.5">{t('sidebar.noTeamsHint')}</p>
          </div>
        )}
      </div>
