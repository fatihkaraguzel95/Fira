...
export function KanbanColumn({ status, tickets }: Props) {
  // Droppable on the WHOLE column container for max hit area
  const { setNodeRef, isOver } = useDroppable({ id: status.id })

  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col min-w-[280px] max-w-[320px] w-full flex-shrink-0 rounded-xl border border-gray-200 shadow-sm transition-colors ${
